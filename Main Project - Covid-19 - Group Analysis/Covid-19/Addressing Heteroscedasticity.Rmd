...
A somewhat perfect distribtuion. 
...
First we replace all instances of 'NA', 'Nan' and 'Inf' as the lm fucntion can't read them, we will replace them with 0 using excel.
```{r}
...
```{r}
attach(coviddata2)
regressionmodel <- lm(logofDeaths ~ logofConfirmed + tests + MaxStringencyIndex + MeanStringencyIndex + MaxFacialCoverings + MeanFacialCoverings + MaxStayHome + MeanStayHome + logofTotalVaccines + logofFullyVacci0ted+ logofTotalBoosters)
```
